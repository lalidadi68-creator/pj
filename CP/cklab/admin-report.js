...
// 2. แก้ไข processLogs ให้คำนวณหา Max Average แทน Max Total
...
function exportReport(mode) {
    // 1. แปลงชื่อโหมดเป็นภาษาไทยสำหรับแสดงในกล่องข้อความ
    const modeNames = {
        'daily': 'รายวัน (Daily)',
    ...
    // 2. ✅ เพิ่มส่วนยืนยัน (Confirm Dialog) ตรงนี้
    // ถ้าผู้ใช้กด "Cancel" (ยกเลิก) ให้หยุดการทำงานทันที (return)
    if (!confirm(`ยืนยันการดาวน์โหลดรายงาน "${selectedModeName}" หรือไม่?`)) {
        return; 
    }
